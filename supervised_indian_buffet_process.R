...
K <- 3
s_fit <- sibp(X, Y$useful, K=K, alpha=10, sigmasq.n=1, a=0.1, b=0.1, sigmasq.A=5, train.ind=i_train, silent=TRUE)
sibp_exclusivity(sibp.fit=s_fit, X=X, num.words=10)
sibp_top_words(s_fit, colnames(X), num.words=5, verbose=FALSE)

# Save fit to file
save(s_fit, file='s_fit_interpretable.RData')
...
write.csv(treatments, 'saved_data/sibp_treatments.csv', row.names=FALSE)


### Parameter sweep ###

# Fit multiple sIBPs using different sets of parameters
sibp_search <- sibp_param_search(X, Y$useful, K=2, alphas=c(6), sigmasq.ns=c(1.0), iters=3, train.ind=i_train)
...
sibp_top_words(sibp_search[['6']][['1']][[1]], colnames(X), 5, verbose=TRUE)
